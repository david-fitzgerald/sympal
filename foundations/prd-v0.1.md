...
|---|---------|----------|------------|-------------|-------|
| 1 | Privacy layer | P0 | Privacy-wrapped integration | High complexity | Core differentiator; approach TBD; may require novel solutions |
...
- **Privacy layer (High)**: This is the hard problem. Approaches floated (PII stripping, prompt fragmentation) need validation. May require experimentation. Risk: might not be achievable with known methods.
...
| Response time | CLI commands respond in <3 seconds for simple queries | User expectation from Claude Code |
...
|------|------------|--------|------------|
| Privacy approach not achievable with known methods | Medium | Critical | Early TDD exploration; accept some exposure if needed |
...
| Privacy mechanism design | Knowledge | Unknown | Yes — TDD must resolve |
| LLM abstraction layer design | Technical | Unknown | No — can start with single provider |

### Open Questions

1. **What is the actual privacy mechanism?** Firewall approach? Prompt fragmentation? Something novel? — Must resolve in TDD
...
|------------|-------------------|
| P1: Privacy & Data Sovereignty | Core feature: privacy layer; all data stored locally; user controls what's sent |
...
**v0.1.0** (2026-01-17) — Initial synthesis:
- Drafted from extraction interview
- Awaiting Principles Checkpoint and Challenge phases
